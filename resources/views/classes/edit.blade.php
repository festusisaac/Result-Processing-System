...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
    ...
    <div class="bg-white shadow overflow-hidden sm:rounded-lg">
        <form action="{{ route('classes.update', $class) }}" method="POST" class="space-y-6 p-6">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <div class="mt-1">
                    <input type="text" name="promoting_class_name" id="promoting_class_name" 
                        value="{{ old('promoting_class_name', $class->promoting_class_name) }}"
            ...
            <div>
                ...
                <div class="mt-1">
                    <input type="text" name="repeating_class_name" id="repeating_class_name" 
                        value="{{ old('repeating_class_name', $class->repeating_class_name) }}"
                        class="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border-gray-300 rounded-md">
                ...
                <div class="mt-1">
                    <select name="subjects[]" id="subjects" multiple
                        ...
                        <option value="{{ $subject->id }}" 
                            {{ in_array($subject->id, old('subjects', $class->subjects->pluck('id')->toArray())) ? 'selected' : '' }}>
                            {{ $subject->name }}
                        </option>
                        @endforeach
                    </select>
