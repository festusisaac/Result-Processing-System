...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
    <div class="mb-6">
        <h1 class="text-2xl font-semibold text-gray-900">Edit Subject Group</h1>
    </div>

    <div class="bg-white shadow overflow-hidden sm:rounded-lg">
        <form action="{{ route('subject-groups.update', $subjectGroup) }}" method="POST" class="space-y-6 p-6">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <div class="mt-1">
                    <input type="text" name="name" id="name" value="{{ old('name', $subjectGroup->name) }}"
            ...
            <div>
                ...
                <div class="mt-1">
                    <select name="session_id" id="session_id" class="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border-gray-300 rounded-md">
                        ...
                        <option value="{{ $session->id }}" {{ (old('session_id', $subjectGroup->session_id) == $session->id) ? 'selected' : '' }}>
                            {{ $session->name }}
                        </option>
                        @endforeach
                    </select>
