...
<div class="max-w-4xl mx-auto">
    ...
    <form action="{{ route('blog.store') }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        @csrf
        ...
        <div class="p-6 space-y-6">
            ...
            <div>
                ...
                <div class="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 border-dashed rounded-lg hover:bg-gray-50 transition-colors cursor-pointer relative" id="drop-zone">
                    ...
                    <img id="preview" class="hidden absolute inset-0 w-full h-full object-cover rounded-lg" />
                </div>
                @error('image')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <textarea name="content" id="content" rows="15" required
                    ...
                    placeholder="Write your post content here... (HTML is supported)">{{ old('content') }}</textarea>
                @error('content')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
