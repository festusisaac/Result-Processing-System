            ...
            <aside :class="sidebarOpen ? 'w-64' : 'w-20'" 
                ...
                <nav class="flex-1 overflow-y-auto py-4 px-3 space-y-1">
                    ...
                    <a href="{{ route('settings.index') }}" 
                       class="flex items-center px-4 py-3 text-gray-300 hover:bg-sidebar-hover hover:text-white transition-all duration-200 rounded-lg group {{ request()->routeIs('settings.*') ? 'bg-indigo-600 text-white shadow-lg' : '' }}">
                ...
                <!-- User Profile -->
                <div class="border-t border-gray-700 p-4 bg-gradient-to-r from-gray-800 to-gray-900" x-data="{ open: false }">
                    <button @click="open = !open" class="flex items-center w-full text-left hover:bg-sidebar-hover rounded-lg p-2 transition-all duration-200">
                        <div class="flex-shrink-0">
                            <div class="h-10 w-10 rounded-full bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center text-white font-bold text-lg shadow-lg">
                    ...
                    <div x-show="open && sidebarOpen" x-collapse class="mt-2">
                        <form method="POST" action="{{ route('logout') }}">
                            @csrf
                            <button type="submit" class="flex items-center w-full px-4 py-2 text-sm text-gray-300 hover:bg-red-600 hover:text-white rounded-lg transition-all duration-200">
